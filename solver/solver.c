...
{
    cube_t canonical;
    cube_orientation_t orientation = cube_detect_orientation_and_normalize(cube, &canonical);
    ...
    int depth;
    int error = findSolutionBasic(cube_str, depth_limit, time_out, moves, &depth);
    ...
    for (int i = 0; i < depth; i++) {
        Move cur         = moves[i];
        face_t emit_face = (options_solver_mode() == OPTIONS_SOLVER_PRESERVE) ? orientation.face_map[cur.orientation]
                                                                              : cur.orientation;
        ...
        if (i != depth - 1) solver_current_solution[len++] = ' ';
...
static void *solver_thread_main (void *arg)
{
    ...
    if (cube->size != 3) {
        snprintf(solver_current_solution, sizeof solver_current_solution, "The algorithm only works on 3x3x3 cubes.");
        is_thread_launched = false;
        return NULL;
    }
    ...
    int error = find_solution_and_update_moves(cube, SOLUTION_DEPTH, SOLVER_TIMEOUT_MS);
    ...
    if (error != 0) {
        if (error == -9) {
            ...
            snprintf(solver_current_solution, sizeof solver_current_solution, "%s", printErrorMessage(error));
        is_thread_launched = false;
        return NULL;
    }

    long long elapsed_ns = (now.tv_sec - start.tv_sec) * (long long)NS_PER_SEC + (now.tv_nsec - start.tv_nsec);
    int elapsed_ms       = (int)(elapsed_ns / NS_PER_MS);
    if (elapsed_ms == 0)
        snprintf(solver_solution_found_text, sizeof solver_solution_found_text,
                 "%d moves solution found in <1 millisecond:", solver_current_solution_size);
    else
        snprintf(solver_solution_found_text, sizeof solver_solution_found_text,
                 "%d moves solution found in ~%d milliseconds:", solver_current_solution_size, elapsed_ms);
...
bool solver_is_searching (void) { return atomic_load(&is_thread_launched); }
...
void solver_apply_current (queue_t *queue, stopwatch_t *timer)
{
    ...
    while (solver_current_solution[i] != '\0') {
        char curr_move = solver_current_solution[i], next_move = solver_current_solution[i + 1];
        if (curr_move == ' ') {
            i++;
            continue;
        }
        if (next_move == '\'') {
            ...
            rotation_t r;
            if (rotation_from_char(curr_move, &r)) queue_push(queue, move_face(r));
            rotation = curr_move;
            i++;
        } else {
            rotation = curr_move;
        ...
        rotation_t r;
        if (rotation_from_char(rotation, &r)) queue_push(queue, move_face(r));
        i++;
    }
}
