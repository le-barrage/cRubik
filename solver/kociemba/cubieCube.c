...
const Edge edgePermutationU[]     = { UB, UR, UF, UL, DR, DF, DL, DB, FR, FL, BL, BR };
...
const Edge edgePermutationR[]     = { FR, UF, UL, UB, BR, DF, DL, DB, DR, FL, BL, UR };
...
const Edge edgePermutationF[]     = { UR, FL, UL, UB, DR, FR, DL, DB, UF, DF, BL, BR };
...
const Edge edgePermutationD[]     = { UR, UF, UL, UB, DF, DL, DB, DR, FR, FL, BL, BR };
...
const Edge edgePermutationL[]     = { UR, UF, BL, UB, DR, DF, FL, DB, FR, UL, DL, BR };
...
const Edge edgePermutationB[]     = { UR, UF, UL, BR, DR, DF, DL, BL, FR, FL, UB, DB };
...
void CubieCube_edgeMultiply (CubieCube *a, CubieCube *b)
{
    ...
    for (int edge = 0; edge < 12; edge++) {
        ...
        eOri[edge]  = (char)((b->edgeOrientation[edge] + a->edgeOrientation[b->edgePermutation[edge]]) % 2);
    }
    for (int e = 0; e < 12; e++) {
...
{
    CubieCube c;
    for (int edge = 0; edge < 12; edge++) c.edgePermutation[cubieCube->edgePermutation[edge]] = edge;
    for (int edge = 0; edge < 12; edge++) c.edgeOrientation[edge] = cubieCube->edgeOrientation[c.edgePermutation[edge]];

    for (int corner = 0; corner < 8; corner++) c.cornerPermutation[cubieCube->cornerPermutation[corner]] = corner;
    for (int corner = 0; corner < 8; corner++) {
        ...
        if (ori >= 3)
            ...
            if (c.cornerOrientation[corner] < 0) c.cornerOrientation[corner] += 3;
        }
    }
    return c;
}
...
{
    short ret = 0;
    for (int i = 0; i < 7; i++) ret = (short)(3 * ret + cubieCube->cornerOrientation[i]);
    return ret;
}
...
{
    short ret = 0;
    for (int i = 0; i < 11; i++) ret = (short)(2 * ret + cubieCube->edgeOrientation[i]);
    return ret;
}
...
short cornerParity (CubieCube *cubieCube)
{
    ...
    for (int i = 7; i >= 0 + 1; i--)
        for (int j = i - 1; j >= URF; j--)
            if (cubieCube->cornerPermutation[j] > cubieCube->cornerPermutation[i]) s++;
    return (short)(s % 2);
}
...
short edgeParity (CubieCube *cubieCube)
{
    ...
    for (int i = 11; i >= 0 + 1; i--)
        for (int j = i - 1; j >= UR; j--)
            if (cubieCube->edgePermutation[j] > cubieCube->edgePermutation[i]) s++;
    return (short)(s % 2);
}
...
short getFRtoBR (CubieCube *cubieCube)
{
    ...
    Edge edge4[4] = { 0 };
    for (int j = BR; j >= UR; j--) {
        if (FR <= cubieCube->edgePermutation[j] && cubieCube->edgePermutation[j] <= BR) {
            a += binomial(11 - j, x + 1);
...
void setFRtoBR (CubieCube *cubieCube, short idx)
{
    ...
    int b            = idx % 24;
    int a            = idx / 24;
    for (int e = UR; e <= BR; e++) cubieCube->edgePermutation[e] = DB;
    for (int j = 1, k; j < 4; j++) {
        k = b % (j + 1);
        b /= j + 1;
        while (k-- > 0) rotateRightEdges(sliceEdge, 0, j);
    }
    x = 3;
    for (int j = UR; j <= BR; j++)
    ...
    x = 0;
    for (int j = UR; j <= BR; j++)
        if (cubieCube->edgePermutation[j] == DB) cubieCube->edgePermutation[j] = otherEdge[x++];
}
...
void setURFtoDLF (CubieCube *cubieCube, short idx)
{
    ...
    int b                = idx % 720;
    int a                = idx / 720;
    for (int c = URF; c <= DRB; c++) cubieCube->cornerPermutation[c] = DRB;
    for (int j = 1, k; j < 6; j++) {
        k = b % (j + 1);
        b /= j + 1;
        while (k-- > 0) rotateRightCorners(corner6, 0, j);
    }
    x = 5;
    for (int j = DRB; j >= 0; j--)
    ...
    x = 0;
    for (int j = URF; j <= DRB; j++)
        if (cubieCube->cornerPermutation[j] == DRB) cubieCube->cornerPermutation[j] = otherCorner[x++];
}
...
void setURtoDF (CubieCube *cubieCube, int idx)
{
    ...
    int b            = idx % 720;
    int a            = idx / 720;
    for (int e = UR; e <= BR; e++) cubieCube->edgePermutation[e] = BR;
    for (int j = 1, k; j < 6; j++) {
        k = b % (j + 1);
        b /= j + 1;
        while (k-- > 0) rotateRightEdges(edge6, 0, j);
    }
    x = 5;
    for (int j = BR; j >= 0; j--)
        if (a - binomial(j, x + 1) >= 0) {
    ...
    x = 0;
    for (int j = UR; j <= BR; j++)
        if (cubieCube->edgePermutation[j] == BR) cubieCube->edgePermutation[j] = otherEdge[x++];
}
...
void setURtoUL (CubieCube *cubieCube, short idx)
{
    ...
    for (int j = 1, k; j < 3; j++) {
        k = b % (j + 1);
        b /= j + 1;
        while (k-- > 0) rotateRightEdges(edge3, 0, j);
    }
    x = 2;
    for (int j = BR; j >= 0; j--)
        if (a - binomial(j, x + 1) >= 0) {
...
short getUBtoDF (CubieCube *cubieCube)
{
    ...
    Edge edge3[3] = { 0 };
    for (int j = UR; j <= BR; j++)
        if (UB <= cubieCube->edgePermutation[j] && cubieCube->edgePermutation[j] <= DF) {
            a += binomial(j, x + 1);
            edge3[x++] = cubieCube->edgePermutation[j];
        }
...
int verify (CubieCube *cubieCube)
{
    int sum           = 0;
    int edgeCount[12] = { 0 };
    for (int e = UR; e <= BR; e++) edgeCount[cubieCube->edgePermutation[e]]++;
    for (int i = 0; i < 12; i++)
        if (edgeCount[i] != 1) return -2;
    for (int i = 0; i < 12; i++) sum += cubieCube->edgeOrientation[i];
    if (sum % 2 != 0) return -3;
    int cornerCount[8] = { 0 };
    for (int c = URF; c <= DRB; c++) cornerCount[cubieCube->cornerPermutation[c]]++;
    for (int i = 0; i < 8; i++)
        if (cornerCount[i] != 1) return -4;
    sum = 0;
    for (int i = 0; i < 8; i++) sum += cubieCube->cornerOrientation[i];
    if (sum % 3 != 0) return -5;
    if ((edgeParity(cubieCube) ^ cornerParity(cubieCube)) != 0) return -6;
