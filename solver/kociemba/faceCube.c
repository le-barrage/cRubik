...
face_t cornerColor[8][3] = {
    { FACE_UP,   FACE_RIGHT, FACE_FRONT },
    { FACE_UP,   FACE_FRONT, FACE_LEFT  },
    { FACE_UP,   FACE_LEFT,  FACE_BACK  },
    { FACE_UP,   FACE_BACK,  FACE_RIGHT },
    { FACE_DOWN, FACE_FRONT, FACE_RIGHT },
    { FACE_DOWN, FACE_LEFT,  FACE_FRONT },
    { FACE_DOWN, FACE_BACK,  FACE_LEFT  },
    { FACE_DOWN, FACE_RIGHT, FACE_BACK  }
};

face_t edgeColor[12][3] = {
    { FACE_UP,    FACE_RIGHT },
    { FACE_UP,    FACE_FRONT },
    { FACE_UP,    FACE_LEFT  },
    { FACE_UP,    FACE_BACK  },
    { FACE_DOWN,  FACE_RIGHT },
    { FACE_DOWN,  FACE_FRONT },
    { FACE_DOWN,  FACE_LEFT  },
    { FACE_DOWN,  FACE_BACK  },
...
FaceCube FaceCube_make (const char *cubeString)
{
    ...
    FaceCube faceCube;
    for (int i = 0; i < 54; i++) faceCube.facelets[i] = getCorrespondingColor(cubeString[i]);
    return faceCube;
}
...
CubieCube FaceCube_toCubieCube (FaceCube *faceCube)
{
    ...
    for (int i = 0; i < 8; i++) cubieCubeToReturn.cornerPermutation[i] = URF;
    for (int i = 0; i < 12; i++) cubieCubeToReturn.edgePermutation[i] = UR;
