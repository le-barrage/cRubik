...
{
    if (error == -1) return "There are not exactly 9 facelets of each color";
    if (error == -2) return "Not all 12 edges exist exactly once";
    if (error == -3) return "Flip error: One edge has to be flipped";
    if (error == -4) return "Not all corners exist exactly once";
    if (error == -5) return "Twist error: One corner has to be twisted";
    if (error == -6) return "Parity error: Two corners or two edges have to be exchanged";
    if (error == -7) return "No solution exists for the given maxDepth";
    if (error == -8) return "Timeout, no solution within given time";
    if (error == -9) return "Cancelled";
    if (error == 1) return "There are not exactly 9 facelets of each color in pattern";
    if (error == 2) return "Not all 12 edges exist exactly once in pattern";
    if (error == 3) return "Flip error: One edge has to be flipped in pattern";
    if (error == 4) return "Not all corners exist exactly once in pattern";
    if (error == 5) return "Twist error: One corner has to be twisted in pattern";
    if (error == 6)
        return "Parity error: Two corners or two edges have to be exchanged in "
               "pattern";
    if (error == 7) return "Cube is already solved";
    return NULL;
}
...
int findSolution (char *cube, int maxDepth, long timeOut, Move moves[maxDepth], char *pattern, int *depth)
{
    ...
    if (errorCode != 0) return abs(errorCode);
    ...
    do {
        ...
        do {
            ...
            // increase power and check if we tried all powers
                ...
                do {
                    ...
                    if (++axis[n] > 5) {
                        ...
                        long long elapsed_time_ns
                            = (now.tv_sec - tStart.tv_sec) * 1000000000LL + (now.tv_nsec - tStart.tv_nsec);
                ...
                } while (n != 0 && (axis[n - 1] == axis[n] || axis[n - 1] == axis[n] + 3));
                // we don't want to twist the same faces or the parallel faces move
                // after move
            } else
        ...
        minDistPhase1[n + 1] = fmax(
            ...
            getPruning(Slice_Flip_Prun, N_SLICE1 * cornersOrientationCoord[n + 1] + slice[n + 1]),
            // according to corners orientations and UD slice
            getPruning(Slice_Twist_Prun, N_SLICE1 * edgesOrientationCoord[n + 1] + slice[n + 1]));
        ...
        if (minDistPhase1[n + 1] == 0 && n >= depthPhase1 - 5) {
            ...
            // == depthPhase1 - 1
            if (n == depthPhase1 - 1 && (depthTotal = totalDepth(depthPhase1, maxDepth)) >= 0) {
...
void createMovesList (Move moves[], int depth)
{
    for (int i = 0; i < depth; i++) {
        moves[i] = Move_createMove(KOCIEMBA_AXIS_TO_FACE[axis[i]], movePower[i]);
    }
}
...
int totalDepth (int depthPhase1, int maxDepth)
{
    ...
    maxDepth = fmin(10, maxDepth - depthPhase1);  // allow only max 10 moves in phase2
    ...
    do {
        do {
            if ((depthPhase1 + depthPhase2 - n > minDistPhase2[n + 1]) && !busy) {
            ...
            } else if ((axis[n] == 0 || axis[n] == 3) ? (++movePower[n] > 3)
                ...
                do {
                    if (++axis[n] > 5) {
                        ...
                        // just move to next axis (increasing done in the if condition)
                        if (axis[n] == 0 || axis[n] == 3)
                            movePower[n] = 1;  // if we move the U or D faces, start from 90 degrees
                        else
                            movePower[n] = 2;  // otherwise, only 180 degrees rotation are allowed
                        busy = false;
                    }
                } while (n != depthPhase1 && (axis[n - 1] == axis[n] || axis[n - 1] == axis[n] + 3));
            } else
                busy = false;
        } while (busy);
        ...
        // get lower bound on the number of moves needed to solve current position
        minDistPhase2[n + 1] = fmax(
            getPruning(Slice_URtoDF_Parity_Prun, (N_SLICE2 * URtoDF[n + 1] + FRtoBR[n + 1]) * 2 + parity[n + 1]),
            getPruning(Slice_URFtoDLF_Parity_Prun, (N_SLICE2 * URFtoDLF[n + 1] + FRtoBR[n + 1]) * 2 + parity[n + 1]));
