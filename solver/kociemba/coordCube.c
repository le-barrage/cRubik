...
void initMergeURtoULandUBtoDF ()
{
    ...
    for (short uRtoUL = 0; uRtoUL < 336; uRtoUL++) {
        for (short uBtoDF = 0; uBtoDF < 336; uBtoDF++) {
            MergeURtoULandUBtoDF[uRtoUL][uBtoDF] = (short)getURtoDFWithIndices(uRtoUL, uBtoDF);
        }
    }
}
...
{
    for (int i = 0; i < N_SLICE2 * N_URFtoDLF * N_PARITY / 2; i++) Slice_URFtoDLF_Parity_Prun[i] = -1;
    int depth = 0;
                        ...
                        default:
                            ...
                            if (getPruning(Slice_URFtoDLF_Parity_Prun,
                                ...
                                setPruning(Slice_URFtoDLF_Parity_Prun,
                                           (N_SLICE2 * newURFtoDLF + newSlice) * 2 + newParity, (char)(depth + 1));
                                done++;
                            }
                    }
                }
...
{
    for (int i = 0; i < N_SLICE2 * N_URtoDF * N_PARITY / 2; i++) Slice_URtoDF_Parity_Prun[i] = -1;
    int depth = 0;
                ...
                for (int j = 0; j < 18; j++) {
                    switch (j) {
                        ...
                        default:
                            ...
                            newParity = parityMove[parity][j];
                            if (getPruning(Slice_URtoDF_Parity_Prun, (N_SLICE2 * newURtoDF + newSlice) * 2 + newParity)
                                == 0x0f) {
...
void initSlice_Twist_Prun ()
{
    ...
    for (int i = 0; i < N_SLICE1 * N_TWIST / 2 + 1; i++) Slice_Twist_Prun[i] = -1;
    int depth = 0;
    // set pruning
...
{
    for (int i = 0; i < N_SLICE1 * N_FLIP / 2; i++) Slice_Flip_Prun[i] = -1;
    int depth = 0;
