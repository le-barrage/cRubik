...
static bool pattern_applies (const pattern_t *p, int cube_size)
{
    if (cube_size < p->min_size) return false;
    if (p->max_size != 0 && cube_size > p->max_size) return false;
    return true;
}
...
bool ui_patterns_draw (int cube_size, queue_t *queue, char *out_text, size_t out_size)
{
    ...
    font_draw("Press 'p' to exit.", GetScreenWidth() - hint_w - HINT_MARGIN, HINT_MARGIN, DEFAULT_FONT_SIZE, DARKGRAY);

    int start_x
        = (GetScreenWidth() - (PATTERN_COLUMNS * PATTERN_BUTTON_W + (PATTERN_COLUMNS - 1) * PATTERN_SPACING)) / 2;
    ...
    for (int i = 0; i < (int)PATTERNS_COUNT; i++) {
        ...
        Rectangle button = (Rectangle){ .x = x, .y = y, .width = PATTERN_BUTTON_W, .height = PATTERN_BUTTON_H };
        bool applicable  = pattern_applies(&PATTERNS[i], cube_size);
        bool hovering    = applicable && CheckCollisionPointRec(GetMousePosition(), button);
        any_hover |= hovering;
