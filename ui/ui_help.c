...
void ui_help_draw (void)
{
    ...
    int font_size = fmax(fmin(floor((float)available_width / chunk_size) * 10, HELP_MAX_FONT_SIZE), HELP_MIN_FONT_SIZE);

    ClearBackground(BACKGROUND_COLOR);
    font_draw("Press 'h' to exit.", HINT_MARGIN, HINT_MARGIN, DEFAULT_FONT_SIZE, DARKGRAY);
    int start_y = GetScreenHeight() / 2 - HELP_TEXTS_COUNT / 2 * HELP_LINE_HEIGHT;
    for (int i = 0; i < HELP_TEXTS_COUNT; i++)
        font_draw(HELP_TEXTS[i], GetScreenWidth() / 2 - font_measure(HELP_TEXTS[i], font_size) / 2,
