...
static int spacing_for (int size)
{
    int s = size / 10;
    return s < 1 ? 1 : s;
}

static const Font *get_font (int size)
{
    for (int i = 0; i < cache_count; i++)
        if (cache[i].size == size) return &cache[i].font;
...
void font_draw (const char *text, int x, int y, int size, Color color)
{
    ...
    if (f)
        DrawTextEx(*f, text, (Vector2){ (float)x, (float)y }, (float)size, (float)spacing_for(size), color);
    else
...
{
    const Font *f = get_font(size);
    if (f) return (int)MeasureTextEx(*f, text, (float)size, (float)spacing_for(size)).x;
