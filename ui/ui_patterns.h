 ...
 * doesn't include `cube_size` are drawn disabled and ignore clicks. If
 * the user clicks an applicable pattern, push its moves onto `queue`,
 * write the formatted move sequence (space-separated tokens) into
 * `out_text`, and return true. Otherwise return false and leave
 * `out_text` untouched. Writes hover state to `*out_hover` (may be
 * NULL) so the caller can aggregate cursor management. */
