...
void ui_loading_draw (int frame_count)
{
    ...
    static const char *DOTS[4] = { "LOADING", "LOADING.", "LOADING..", "LOADING..." };
    BeginDrawing();
    ClearBackground(BACKGROUND_COLOR);
    int text_w = font_measure("LOADING...", LOADING_FONT_SIZE);
    font_draw(DOTS[phase], GetScreenWidth() / 2 - text_w / 2, GetScreenHeight() / 2 - DEFAULT_FONT_SIZE,
              LOADING_FONT_SIZE, BLACK);
    EndDrawing();
}
