...
static int draw_text_cursor_after (const char *s, int font_size)
{
...
void ui_moves_draw (const char *text, float font_size, int y, int highlight_token)
{
    ...
    int token_count = 0;
    for (char *t = strtok(work_copy, " "); t && token_count < MOVES_MAX_TOKENS; t = strtok(NULL, " "))
    ...
    for (int i = 1; i < token_count; i++) {
        ...
        if (font_measure(line_buf, font_size) > max_width && line_count < MOVES_MAX_LINES) {
            line_buf[line_len]       = '\0';
    ...
    for (int line = 0; line < line_count; line++) {
        ...
        if (highlight_token < start_idx || highlight_token >= end_idx) continue;

        int h_offset = 0;
        for (int i = start_idx; i < highlight_token; i++) h_offset += strlen(tokens[i]) + 1;
