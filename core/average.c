...
static void get_solves_filename (char *out, int cube_size)
{
 ...
 * undefined otherwise. */
static int time_to_ms (const char *time)
{
    int seconds = 600 * (time[0] - '0') + 60 * (time[1] - '0') + 10 * (time[3] - '0') + (time[4] - '0');
    return MS_PER_SEC * seconds + 100 * (time[6] - '0') + 10 * (time[7] - '0') + (time[8] - '0');
}
...
static void time_format (int total_ms, char *out, size_t out_size)
{
...
static cJSON *read_json_file (const char *filename)
{
    ...
    if (!json) {
        ...
        if (err) LOG_ERROR("%s: parse error: %s", filename, err);
    }
    return json;
}

static int write_json_file (const char *filename, cJSON *json)
{
...
static void ensure_cache_loaded (int cube_size)
{
    if (cached_root && cached_cube_size == cube_size) return;
    clear_cache();
...
static cJSON *get_cached_solves_array (int cube_size)
{
    ensure_cache_loaded(cube_size);
    if (!cached_root) return NULL;
    cJSON *solves = cJSON_GetObjectItemCaseSensitive(cached_root, "solves");
    if (!cJSON_IsArray(solves)) {
        LOG_ERROR("cached file for size %d: 'solves' array missing or wrong type", cube_size);
        return NULL;
    }
    return solves;
}

void solves_shutdown (void) { clear_cache(); }

static void parse_solve_json (const cJSON *solve, solve_entry_t *out)
{
    *out = (solve_entry_t){ .valid = false, .dnf = false, .ms = -1 };
    ...
    if (!cJSON_IsString(time) || !time->valuestring) return;
    out->valid = true;
...
static void wrap_in_parens (char *str)
{
...
static void mark_best_worst (solve_entry_t entries[LAST_N_SOLVES])
{
    int best_idx   = -1;
    int worst_idx  = -1;
    int best_ms    = -1;
    ...
    if (best_idx >= 0) wrap_in_parens(entries[best_idx].display);
    if (worst_idx >= 0 && worst_idx != best_idx) wrap_in_parens(entries[worst_idx].display);
}

static int add_solve_entry (cJSON *root, const char *time, const char *scramble)
{
    cJSON *solves = cJSON_GetObjectItemCaseSensitive(root, "solves");
    if (!cJSON_IsArray(solves)) return 1;

    cJSON *solve = cJSON_CreateObject();
    if (!solve) return 1;

    if (!cJSON_AddStringToObject(solve, "time", time) || !cJSON_AddStringToObject(solve, "scramble", scramble)
        || !cJSON_AddBoolToObject(solve, "dnf", 0) || !cJSON_AddBoolToObject(solve, "plus_two", 0)) {
        cJSON_Delete(solve);
        return 1;
    }
...
static cJSON *new_solves_root (int cube_size)
{
    cJSON *root = cJSON_CreateObject();
    if (!root) return NULL;
    char puzzle_name[20];
    snprintf(puzzle_name, sizeof puzzle_name, "%dx%dx%d", cube_size, cube_size, cube_size);
...
void solves_save (const char *time, const char *scramble, int cube_size)
{
    ...
    if (!cached_root) {
        ...
        if (probe) {
            ...
            fclose(probe);
            if (size > 0) {
                LOG_ERROR("%s: refusing to overwrite unparseable file", filename);
                return;
            }
        }
...
{
    for (int i = 0; i < LAST_N_SOLVES; i++) strcpy(times[i], "-");
    ...
    for (int i = 0; i < n; i++) parse_solve_json(cJSON_GetArrayItem(solves, start + i), &entries[i]);

    if (n == LAST_N_SOLVES) mark_best_worst(entries);

    for (int i = 0; i < LAST_N_SOLVES; i++)
        if (entries[i].valid) strcpy(times[i], entries[i].display);
}

/* Loads the most recent up-to-`n` solves into `out`. Returns the number
 * actually loaded (0..n). Slots past the loaded count are left
 * default-initialized. */
static int load_last_n_entries (int n, int cube_size, solve_entry_t *out)
{
    for (int i = 0; i < n; i++) out[i] = (solve_entry_t){ 0 };
    ...
    for (int i = 0; i < count; i++) parse_solve_json(cJSON_GetArrayItem(solves, start + i), &out[i]);
...
static void compute_average_drop_extremes (const solve_entry_t *entries, int n, char *out, size_t out_size)
{
    int dnf_count = 0;
    for (int i = 0; i < n; i++)
        if (entries[i].dnf) dnf_count++;
    if (dnf_count > 1) {
        strcpy(out, "DNF");
        return;
    }
...
void solves_best_time (int cube_size, char out[AVG_STR_LEN])
{
    ...
    for (int i = 0; i < total; i++) {
        ...
        if (!entry.valid || entry.dnf) continue;
        if (best_ms < 0 || entry.ms < best_ms) best_ms = entry.ms;
    }

    if (best_ms >= 0) time_format(best_ms, out, AVG_STR_LEN);
}

static int target_index (int index, int total_solves)
{
    if (total_solves < LAST_N_SOLVES) return index;
    return total_solves - LAST_N_SOLVES + index;
}

static int set_dnf (cJSON *solve, bool value)
{
    cJSON_ReplaceItemInObject(solve, "dnf", value ? cJSON_CreateTrue() : cJSON_CreateFalse());
    return 0;
}

static int set_plus_two (cJSON *solve, bool value)
{
    cJSON *plus_two = cJSON_GetObjectItemCaseSensitive(solve, "plus_two");
    cJSON *time_obj = cJSON_GetObjectItemCaseSensitive(solve, "time");
    if (!cJSON_IsString(time_obj) || !time_obj->valuestring) return 1;
    ...
    cJSON_ReplaceItemInObject(solve, "plus_two", value ? cJSON_CreateTrue() : cJSON_CreateFalse());
    return 0;
}

static void modify_solve_field (int index, int cube_size, field_type_t field)
{
    ...
    if (idx < 0 || idx >= total) {
        LOG_ERROR("size %d: solve index %d out of range (total %d)", cube_size, idx, total);
        return;
    }
    ...
    if (status == 0) flush_cache();
}

void solves_toggle_dnf (int index, int cube_size) { modify_solve_field(index, cube_size, FIELD_DNF); }

void solves_toggle_plus_two (int index, int cube_size) { modify_solve_field(index, cube_size, FIELD_PLUS_TWO); }
...
bool solves_get_scramble (int last_n_index, int cube_size, char *out, size_t out_size)
{
    ...
    if (!cJSON_IsString(scramble) || !scramble->valuestring) {
        LOG_ERROR("size %d: solve at index %d missing 'scramble' field", cube_size, idx);
        return false;
    }
    if (strlen(scramble->valuestring) + 1 > out_size) {
        LOG_WARN("solves_get_scramble: out_size %zu too small for scramble (%zu)", out_size,
                 strlen(scramble->valuestring) + 1);
        return false;
    }
