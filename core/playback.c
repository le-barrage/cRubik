...
static int
count_pops_and_tokens (const char *text,
...
void
playback_init (const char *text)
{
  strncpy (playback.text, text, sizeof (playback.text) - 1);
  playback.text[sizeof (playback.text) - 1] = '\0';
  playback.token_count
      = count_pops_and_tokens (playback.text, playback.pops_per_token);
  ...
  if (playback.pops_remaining <= 0)
    {
      ...
      if (playback.current_move_index < playback.token_count)
        playback.pops_remaining
            = playback.pops_per_token[playback.current_move_index];
    }
  playback.pops_remaining--;
}
