...
static bool are_opposite_faces (char a, char b)
{
...
static char extract_face_letter (const char *full_move)
{
...
static bool move_is_valid (const char *full_move, char *const *sequence, int sequence_length)
{
    char fm_face   = extract_face_letter(full_move);
    char last_face = extract_face_letter(sequence[sequence_length - 1]);

    if (fm_face == last_face) return false;

    if (sequence_length > 1) {
        char prev_face = extract_face_letter(sequence[sequence_length - 2]);
        if (fm_face == prev_face && are_opposite_faces(fm_face, last_face)) return false;
    }
    return true;
}

static void free_partial (char **sequence, int count)
{
...
{
    if (cube_size < 1) return 0;
    if (cube_size <= 2) return MIN_SCRAMBLE_LEN;
    if (cube_size > LARGE_CUBE_THRESHOLD) return MAX_SCRAMBLE_LEN;
    return MOVES_PER_LAYER * (cube_size - 2);
}

scramble_status_t scramble_generate (char **sequence, int length, int cube_size)
{
    if (cube_size < 1 || length < 1) return SCRAMBLE_BAD_SIZE;

    for (int i = 0; i < length; i++) sequence[i] = NULL;

    const char *const *moves = (cube_size == 2) ? MOVES_2X2 : MOVES_3X3;
    int move_count           = (cube_size == 2) ? (int)ARRAY_LEN(MOVES_2X2) : (int)ARRAY_LEN(MOVES_3X3);
    ...
    while (seq_len < length) {
        ...
        if (seq_len > 0 && !move_is_valid(full_move, sequence, seq_len)) continue;
