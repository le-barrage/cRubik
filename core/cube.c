...
static bool is_inner_cubie (int x, int y, int z, int size)
{
    ...
    for (int x = 0; x < size; x++) {
        ...
        for (int y = 0; y < size; y++) {
            ...
            for (int z = 0; z < size; z++) {
                if (is_inner_cubie(x, y, z, size)) continue;
...
cube_t cube_deep_copy (const cube_t *src)
{
    ...
    for (int x = 0; x < src->size; x++) {
        ...
        for (int y = 0; y < src->size; y++) {
            ...
            memcpy(dst.cube[x][y], src->cube[x][y], src->size * sizeof(cubie_t));
        }
    }
...
{
    if (cube == NULL || cube->cube == NULL) return;
    for (int x = 0; x < cube->size; x++) {
        for (int y = 0; y < cube->size; y++) free(cube->cube[x][y]);
        free(cube->cube[x]);
    }
    free(cube->cube);
    cube->cube = NULL;
...
static void step_animation (cube_t *cube, int rotation_speed)
{
    if (cube->is_animating) cube->rotation_degrees += rotation_speed;
    if (cube->rotation_degrees > ROTATION_DEGREES_FULL) {
        cube->rotation_degrees = 0;
        cube->is_animating     = false;
        if (cube->current_move.single_layer)
            cube_rotate_single_layer(cube, cube->current_move.rotation, cube->current_move.num_layers);
        else
            cube_rotate(cube, cube->current_move.rotation, cube->current_move.num_layers);
...
static bool is_in_layer (int pos[3], int axis_dim, layer_pos_t kind, const move_t *m, int size)
{
    ...
    switch (kind) {
        ...
        case LAYER_INDEX_ZERO:
            if (m->single_layer) return pos[axis_dim] == n - 1;
            return pos[axis_dim] < n;
        case LAYER_INDEX_LAST:
            if (m->single_layer) return pos[axis_dim] == size - n;
...
static Vector3 cubie_rotation_axis (const move_t *m, int pos_x, int pos_y, int pos_z, int size)
{
    if (m->rotation < 0 || m->rotation >= (int)ARRAY_LEN(ROTATION_AXES)) return (Vector3){ 0, 0, 0 };
    const rotation_axis_def_t *def = &ROTATION_AXES[m->rotation];
    int pos[3]                     = { pos_x, pos_y, pos_z };
    if (!is_in_layer(pos, def->axis_dim, def->kind, m, size)) return (Vector3){ 0, 0, 0 };
    return def->axis;
}

static void draw_one_cubie (cube_t *cube, int pos_x, int pos_y, int pos_z)
{
    ...
    Vector3 axis         = cubie_rotation_axis(&cube->current_move, pos_x, pos_y, pos_z, cube->size);
    bool axis_is_zero    = (axis.x == 0 && axis.y == 0 && axis.z == 0);
    int rotation_degrees = axis_is_zero ? 0 : cube->rotation_degrees;
    cubie_draw(&cube->cube[pos_x][pos_y][pos_z], position, axis, rotation_degrees);
}
    ...
    for (int x = 0; x < cube->size; x++)
        for (int y = 0; y < cube->size; y++)
            for (int z = 0; z < cube->size; z++) {
                if (is_inner_cubie(x, y, z, cube->size)) continue;
...
static char face_letter_from_move (const char *move, size_t len)
{
    if (move[len - 1] == '\'') return tolower((unsigned char)move[len - 2]);
    if (move[len - 1] == '2') return move[len - 2];
    return move[len - 1];
}
...
void cube_apply_move (cube_t *cube, const char *move)
{
    ...
    if (move[len - 1] == '2') cube_rotate(cube, r, num_layers);
...
static int resolved_x (int dir_x, int i) { return (dir_x == -1) ? i : dir_x; }

static int resolved_y (int dir_x, int dir_y, int i, int j) { return (dir_y == -1) ? (dir_x == -1) ? j : i : dir_y; }

static int resolved_z (int dir_z, int j) { return (dir_z == -1) ? j : dir_z; }

static void extract_face_and_rotate_cubies (cube_t *cube, Vector3 dir, void (*cubie_rotation)(cubie_t *), int size,
...
static void transpose_face (int size, cubie_t face[size][size])
{
    for (int i = 0; i < size; i++)
...
static void reverse_face_rows (int size, cubie_t face[size][size])
{
    for (int i = 0; i < size; i++)
...
static void reverse_face_columns (int size, cubie_t face[size][size])
{
    for (int j = 0; j < size; j++)
...
static void write_face_back (cube_t *cube, Vector3 dir, int size, cubie_t face[size][size])
{
    for (int i = 0; i < size; i++)
        for (int j = 0; j < size; j++) {
            int x               = resolved_x(dir.x, i);
...
static void rotate_layer (cube_t *cube, Vector3 dir, void (*cubie_rotation)(cubie_t *), bool anti_clockwise)
{
    int size = cube->size;
    cubie_t face[size][size];
...
static Vector3 layer_axis_vector (int axis_dim, int layer_idx)
{
    switch (axis_dim) {
        case AXIS_X:
            return (Vector3){ layer_idx, -1, -1 };
        case AXIS_Y:
            return (Vector3){ -1, layer_idx, -1 };
        case AXIS_Z:
            return (Vector3){ -1, -1, layer_idx };
    }
...
void cube_rotate (cube_t *cube, rotation_t rotation, int num_layers)
{
    if (rotation >= ROT_X && rotation <= ROT_Z_PRIME) {
        ...
        for (int k = 0; k < 3; k++) cube_rotate(cube, parts[k], half);
        return;
    }

    if (rotation < 0 || rotation >= (int)ARRAY_LEN(LAYER_ROTATIONS)) return;
...
void cube_rotate_single_layer (cube_t *cube, rotation_t rotation, int depth)
{
    if (rotation >= ROT_M || depth < 1 || depth > cube->size / 2) return;
    const layer_rotation_def_t *def = &LAYER_ROTATIONS[rotation];
    int layer_idx                   = (def->iter == LAYER_ITER_FROM_LAST) ? cube->size - depth : depth - 1;
    Vector3 dir                     = layer_axis_vector(def->axis_dim, layer_idx);
    rotate_layer(cube, dir, def->cubie_rotation, def->anti_clockwise);
}
...
static char face_letter_from_color (cubie_t cubie, face_t face)
{
...
void cube_to_string (const cube_t *cube, char *out, size_t out_size)
{
    ...
    int idx  = 0;
    for (int z = 0; z < size; z++)
        for (int x = 0; x < size; x++) out[idx++] = face_letter_from_color(cube->cube[x][size - 1][z], FACE_UP);
    for (int x = size - 1; x >= 0; x--)
        for (int z = size - 1; z >= 0; z--) out[idx++] = face_letter_from_color(cube->cube[size - 1][x][z], FACE_RIGHT);
    for (int z = size - 1; z >= 0; z--)
        for (int x = 0; x < size; x++) out[idx++] = face_letter_from_color(cube->cube[x][z][size - 1], FACE_FRONT);
    for (int z = size - 1; z >= 0; z--)
        for (int x = 0; x < size; x++) out[idx++] = face_letter_from_color(cube->cube[x][0][z], FACE_DOWN);
    for (int x = size - 1; x >= 0; x--)
        for (int z = 0; z < size; z++) out[idx++] = face_letter_from_color(cube->cube[0][x][z], FACE_LEFT);
    for (int z = size - 1; z >= 0; z--)
        for (int x = size - 1; x >= 0; x--) out[idx++] = face_letter_from_color(cube->cube[x][z][0], FACE_BACK);
    out[idx] = '\0';
}
...
static Color world_center_color (const cube_t *cube, face_t face)
{
    int mid = cube->size / 2;
    int max = cube->size - 1;
    switch (face) {
        case FACE_UP:
            return cube->cube[mid][max][mid].colors[FACE_UP];
        case FACE_DOWN:
            return cube->cube[mid][0][mid].colors[FACE_DOWN];
        case FACE_FRONT:
            return cube->cube[mid][mid][max].colors[FACE_FRONT];
        case FACE_BACK:
            return cube->cube[mid][mid][0].colors[FACE_BACK];
        case FACE_LEFT:
            return cube->cube[0][mid][mid].colors[FACE_LEFT];
        case FACE_RIGHT:
            return cube->cube[max][mid][mid].colors[FACE_RIGHT];
    }
    return BLACK;
}

static face_t find_world_face_showing (const cube_t *cube, Color target)
{
    static const face_t all[FACE_COUNT] = {
        FACE_UP, FACE_FRONT, FACE_RIGHT, FACE_BACK, FACE_LEFT, FACE_DOWN,
    };
    for (int i = 0; i < FACE_COUNT; i++)
        if (colors_equal(world_center_color(cube, all[i]), target)) return all[i];
    return FACE_UP;
}

cube_orientation_t cube_detect_orientation_and_normalize (const cube_t *src, cube_t *out_canonical)
{
    cube_orientation_t o = { .count = 0 };
    for (int f = 0; f < FACE_COUNT; f++) o.face_map[f] = find_world_face_showing(src, CANONICAL_COLOR[f]);
...
static char rotation_letter (rotation_t r)
{
    switch (r) {
        case ROT_X:
        case ROT_X_PRIME:
...
static bool rotation_is_inverse (rotation_t r) { return r == ROT_X_PRIME || r == ROT_Y_PRIME || r == ROT_Z_PRIME; }

void cube_append_normalization_tokens (char *buf, const cube_orientation_t *o)
{
    ...
    for (int i = 0; i < o->count;) {
        ...
        if (i + 1 < o->count && o->moves[i + 1] == r) {
            ...
            buf[len++] = letter;
            if (rotation_is_inverse(r)) buf[len++] = '\'';
            buf[len++] = ' ';
            i++;
        }
    }
