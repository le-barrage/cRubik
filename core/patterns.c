...
static void
append_token (char *out, size_t out_size, size_t *pos, bool *wrote_first,
              const char *tok)
{
...
void
pattern_emit_static (const move_t *arr, size_t n, queue_t *q, char *out,
                     size_t out_size)
{
  size_t pos = 0;
  ...
  for (size_t j = 0; j < n; j++)
    {
      ...
      bool is_pair_second
          = (j > 0 && arr[j].rotation == arr[j - 1].rotation);
      if (is_pair_second)
        continue;

      bool is_half
          = (j + 1 < n && arr[j + 1].rotation == arr[j].rotation);
...
static const move_t CROSS[] = {
  MOVE_FACE (ROT_R),       MOVE_FACE (ROT_R),
  MOVE_FACE (ROT_L_PRIME), MOVE_FACE (ROT_D),
  MOVE_FACE (ROT_F),       MOVE_FACE (ROT_F),
  MOVE_FACE (ROT_R_PRIME), MOVE_FACE (ROT_D_PRIME),
  MOVE_FACE (ROT_R_PRIME), MOVE_FACE (ROT_L),
  MOVE_FACE (ROT_U_PRIME), MOVE_FACE (ROT_D),
  MOVE_FACE (ROT_R),       MOVE_FACE (ROT_D),
  MOVE_FACE (ROT_B),       MOVE_FACE (ROT_B),
  MOVE_FACE (ROT_R_PRIME), MOVE_FACE (ROT_U),
  MOVE_FACE (ROT_D),       MOVE_FACE (ROT_D),
};

static const move_t FOUR_CROSSES[] = {
  MOVE_FACE (ROT_U), MOVE_FACE (ROT_U),
  MOVE_FACE (ROT_R), MOVE_FACE (ROT_R),
  MOVE_FACE (ROT_L), MOVE_FACE (ROT_L),
  MOVE_FACE (ROT_F), MOVE_FACE (ROT_F),
  MOVE_FACE (ROT_B), MOVE_FACE (ROT_B),
  MOVE_FACE (ROT_D), MOVE_FACE (ROT_D),
  MOVE_FACE (ROT_L), MOVE_FACE (ROT_L),
...
static const move_t CUBE_IN_CUBE[] = {
  MOVE_FACE (ROT_F),       MOVE_FACE (ROT_L),
  MOVE_FACE (ROT_F),       MOVE_FACE (ROT_U_PRIME),
  MOVE_FACE (ROT_R),       MOVE_FACE (ROT_U),
  MOVE_FACE (ROT_F),       MOVE_FACE (ROT_F),
  MOVE_FACE (ROT_L),       MOVE_FACE (ROT_L),
  MOVE_FACE (ROT_U_PRIME), MOVE_FACE (ROT_L_PRIME),
  MOVE_FACE (ROT_B),       MOVE_FACE (ROT_D_PRIME),
  MOVE_FACE (ROT_B_PRIME), MOVE_FACE (ROT_L),
  MOVE_FACE (ROT_L),       MOVE_FACE (ROT_U),
};

static const move_t CUBE_IN_CUBE_IN_CUBE[] = {
  MOVE_FACE (ROT_U_PRIME), MOVE_FACE (ROT_L_PRIME),
  MOVE_FACE (ROT_U_PRIME), MOVE_FACE (ROT_F_PRIME),
  MOVE_FACE (ROT_R),       MOVE_FACE (ROT_R),
  MOVE_FACE (ROT_B_PRIME), MOVE_FACE (ROT_R),
  MOVE_FACE (ROT_F),       MOVE_FACE (ROT_U),
  MOVE_FACE (ROT_B),       MOVE_FACE (ROT_B),
  MOVE_FACE (ROT_U),       MOVE_FACE (ROT_B_PRIME),
  MOVE_FACE (ROT_L),       MOVE_FACE (ROT_U_PRIME),
  MOVE_FACE (ROT_F),       MOVE_FACE (ROT_U),
  MOVE_FACE (ROT_R),       MOVE_FACE (ROT_F_PRIME),
};

static const move_t FOUR_SPOTS[] = {
  MOVE_FACE (ROT_F),       MOVE_FACE (ROT_F),
  MOVE_FACE (ROT_B),       MOVE_FACE (ROT_B),
  MOVE_FACE (ROT_U),       MOVE_FACE (ROT_D_PRIME),
  MOVE_FACE (ROT_R),       MOVE_FACE (ROT_R),
  MOVE_FACE (ROT_L),       MOVE_FACE (ROT_L),
  MOVE_FACE (ROT_U),       MOVE_FACE (ROT_D_PRIME),
};
...
static const move_t SIX_SPOTS[] = {
  MOVE_FACE (ROT_U),       MOVE_FACE (ROT_D_PRIME),
  MOVE_FACE (ROT_R),       MOVE_FACE (ROT_L_PRIME),
  MOVE_FACE (ROT_F),       MOVE_FACE (ROT_B_PRIME),
  MOVE_FACE (ROT_U),       MOVE_FACE (ROT_D_PRIME),
};

static void
build_superflip (int size, queue_t *q, char *out, size_t out_size)
{
...
static void
build_checkerboard (int size, queue_t *q, char *out, size_t out_size)
{
      ...
      for (size_t s = 0; s < ARRAY_LEN (slices); s++)
        {
          ...
          if (thickness == 1)
            ...
            snprintf (buf, sizeof buf, "%dw%c2", thickness,
                      slices[s].letter);
          append_token (out, out_size, &pos, &wrote_first, buf);
        }
    }
...
static void
build_cross (int size, queue_t *q, char *out, size_t out_size)
{
  (void)size;
...
static void
build_four_crosses (int size, queue_t *q, char *out, size_t out_size)
{
  (void)size;
  pattern_emit_static (FOUR_CROSSES, ARRAY_LEN (FOUR_CROSSES), q, out,
                       out_size);
}

static void
build_cube_in_cube (int size, queue_t *q, char *out, size_t out_size)
{
  (void)size;
  pattern_emit_static (CUBE_IN_CUBE, ARRAY_LEN (CUBE_IN_CUBE), q, out,
                       out_size);
}

static void
build_cube_in_cube_in_cube (int size, queue_t *q, char *out, size_t out_size)
{
  (void)size;
  pattern_emit_static (CUBE_IN_CUBE_IN_CUBE, ARRAY_LEN (CUBE_IN_CUBE_IN_CUBE),
                       q, out, out_size);
}

static void
build_four_spots (int size, queue_t *q, char *out, size_t out_size)
{
  (void)size;
...
static void
build_six_spots (int size, queue_t *q, char *out, size_t out_size)
{
  (void)size;
