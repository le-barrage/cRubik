...
void timer_update (stopwatch_t *timer)
{
    ...
    long long elapsed_ms = (now.tv_sec - timer->start_time.tv_sec) * (long long)MS_PER_SEC
...
void timer_disable (stopwatch_t *timer) { timer->is_disabled = true; }

void timer_enable (stopwatch_t *timer) { timer->is_disabled = false; }
