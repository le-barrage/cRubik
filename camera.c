...
Camera camera = { { 0 }, { 0, 0, 0 }, { 0, 1, 0 }, CAMERA_FOVY,
                  CAMERA_PERSPECTIVE };

void
camera_init (int cube_size)
{
...
void
camera_update (int cube_size)
{
  if (IsMouseButtonPressed (MOUSE_BUTTON_MIDDLE))
    camera_init (cube_size);

  float dt = GetFrameTime ();

  camera_mag += camera_mag_vel * dt;
  if (camera_mag < CAMERA_MIN_MAG_FACTOR * cube_size)
    camera_mag = CAMERA_MIN_MAG_FACTOR * cube_size;
  if (camera_mag > CAMERA_MAX_MAG_FACTOR * cube_size)
    camera_mag = CAMERA_MAX_MAG_FACTOR * cube_size;
  ...
  if (camera_phi >= PI)
    camera_phi = PI - CAMERA_PHI_EPSILON;
  if (camera_phi <= 0)
    camera_phi = CAMERA_PHI_EPSILON;
